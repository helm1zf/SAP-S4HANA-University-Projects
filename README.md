# SAP-S4HANA-University-Projects
My practical work and  notes from the SAP modules at SAP modules at Trier University. Focus on S/4HANA basics and system setup.


# SAP S/4HANA: My University Lab Records.
**Grade: 1.0** | Projects from my Bachelor at Trier University of Applied Sciences.

## What is This?
I created this repository to keep a record of what I learned in my SAP modules. I didn't want to lose the knowledge just because the university semester ended.

These notes are based on the practical lab work I did using the "Global Bike" system. It shows that I understand how the system is put together and how the business data flows.

### 1. Setting up the Company (Customizing)
I followed the lab manuals to build a company structure. I practiced creating the "basics" that company needs to run:
* Defining the **Company Code** and **Plants**
* Linking the storage locations so the system knows where the inventory is.
* Setting up the **Material Ledger** (using code like OMX1 and OMX3) so that the finance side could actually talk to the warehouse side.

### 2. Running the Business Cycles
I spent a lot of time practicing the standard "demo" flows:
* **Buying side (Purchasing):** Creating Purchase Orders,  doing the Good Receipt (MIGO), and checking the vendor invoices.
* **Selling side (Sales):** Handling customer orders and seeing how the system checks if we have enough bikes in stock.
* **Master Data:** I learned that you can't do anything in SAP without a "Business Partner" (Vendor/Customer) and a "Material Master" first.

### 3. Modern SAP (Fiori)
Most of my work was in **SAP Fiori**. I practiced using the "Apps" and "Tiles" instead of the old transaction codes. It made the system feel a lot more like a modern app.

## Why this matter to me
Getting a 1.0 was great, but the real value was seeing how a single click in the warehouse affects the accounting balance sheet. It taught me how "Integrated" a big company really is.
